...
export const messages = {
  ...
  // Экран открыт по несуществующему id. Раньше на этом месте стоял 'common.none' («—»):
  // после слайса #78 текст пустого состояния стал заголовком 16/600, и прочерк в нём читался
  // как поломка вёрстки, а не как «объекта нет».
  ...
  // Канон §15 / слайс #72: короткая подпись — «Задолженности» не влезала в сегмент-контрол
  // без ужатия. Строка мода-нейтральна (нет лексики «урок/ученик»), поэтому `_client`-пары
  // у неё нет — как и у соседней `finance.tab.debts`.
  'analytics.debts': 'Долги',
  ...
  // Слово знака живёт в брендовом слое (`ui/brand/Wordmark`), а не в словаре: имя бренда
  // не переводится и не переключается модом. Ключ оставлен для заголовков/a11y.
  'auth.title': 'Tutor+',
  'auth.subtitle': 'Расписание, ученики и оплаты — в одном месте',
  'auth.subtitle_client': 'Расписание, клиенты и оплаты — в одном месте',
  // Таглайн канона (слайс #77). `_client`-пары нет намеренно: «практика» — маркетинговый
  // зонт над обоими режимами (дизайн-система v2 §12), формулировка для клиента та же.
  'auth.tagline': 'Плюс к вашей практике',
  'auth.signIn': 'Войти',
  ...
  // Домен отказал в правке (ADR-0016 §4) — три РАЗНЫЕ причины, каждая со своим текстом:
  // деньги на занятии, уже проведённое занятие, пустое окно охвата (TP-FIX-0719, п. 3).
  ...
  // ── Лента: вкладки read-состояния + чипы категорий (TP-FIX-0719, п. 7) ────
  // «Прочитанные»/«Тип»/«Фильтр уведомлений» ушли вместе с модальным фильтром.
  ...
  // Пустое имя профиля — приглашение вместо прочерка (TP-FIX-0719, п. 4).
  'set.namePlaceholder': 'Добавьте имя',
